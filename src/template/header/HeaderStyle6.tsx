...
import { 
  ...
  Wallet, ShieldCheck, Gamepad2, ChevronDown, Menu, X, Gem
} from 'lucide-react';
...
export default function HeaderStyle6({
  config,
  user,
  onOpenAuth,
  onNavigate,
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6">
            ...
            {user ? (
              <div className="relative">
                <button
                  ...
                  className="flex items-center gap-2 bg-[#1D4ED8] hover:bg-blue-800 text-white font-black px-6 py-2.5 rounded-lg text-xs transition-all shadow-md cursor-pointer"
                >
                  <UserIcon size={15} />
                  <span>{user.name}</span>
              ...
              <button
                ...
                className="bg-[#1D4ED8] hover:bg-blue-800 text-white font-black text-sm px-8 py-2.5 rounded-lg shadow-md transition-all cursor-pointer"
              >
                Login
              </button>
            )}
        ...
        {isMobileMenuOpen && (
          <div className="md:hidden pb-4 pt-2 border-t border-slate-100 space-y-2">
            ...
            {!user && (
              <button onClick={() => { onOpenAuth('login'); setIsMobileMenuOpen(false); }} className="w-full bg-[#1D4ED8] text-white font-extrabold py-2.5 rounded-lg text-xs mt-2">
                Login
              </button>
            )}
          </div>
